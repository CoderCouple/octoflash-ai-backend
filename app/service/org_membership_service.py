"""OrgMembershipService — invite / list / change role / remove.

Owners can do anything; admins can manage members but cannot modify owners.
Seat count is kept in sync with Stripe on every membership mutation
(no-op until Stripe is configured).
"""
...
class OrgMembershipService:
    ...
    async def _active_member_count(self, org_id: str) -> int:
        members, _ = await self.membership_repo.list_by_org(org_id, 0, 10000)
    ...
    async def invite_member(
        ...
        try:
            await self.billing_service.sync_seat_count(
                org_id, await self._active_member_count(org_id)
            )
        except Exception:
            logger.warning("Seat sync skipped for org=%s", org_id)
    ...
    async def change_role(
        ...
        if (
            membership.role == OrgRole.OWNER.value
            and actor_role != OrgRole.OWNER.value
        ):
            raise HTTPException(
